...
- Production deployments must provide RSA signing key material and a 32-byte document master key; development can generate fallback signing keys for setup speed.
...
- Development fallback RSA signing keys reduce setup friction, but production requires explicit RSA key material from a secure secret source.
...
Begin multi-institution support with role-scoped dashboards and data isolation.
